...
基于[Hexo]默认主题light和主题chenall V2.2修改而成,采用bootstrap css框架,主要考虑多站点支持.
...
```
git clone https://github.com/chalecao/chalecao.git themes/chale
```
...
```
{% ijs %}
return link('http://haomou.net');
{% endijs %}
```

以上通过hexo内置的helper插件link实现在文章中插入一个链接指向`http://haomou.net`.
